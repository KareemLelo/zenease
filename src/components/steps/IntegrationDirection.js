...
const FieldMapping = () => {
  ...
  useEffect(() => {
    if (selectedFields.length > 0 && externalSystemFields.length > 0) {
      const suggestions = {};
      
      selectedFields.forEach(zenhrFieldId => {
        // Get ZenHR field
        const zenhrField = availableFields.find(f => f.id === zenhrFieldId);
        if (!zenhrField) return;
        
        // Look for matching external fields by name similarity
        const zenhrName = zenhrField.name.toLowerCase();
        const zenhrId = zenhrField.id.toLowerCase();
        
        // Try to find matches
        const potentialMatches = externalSystemFields.filter(extField => {
          const extName = extField.name.toLowerCase();
          const extId = extField.id.toLowerCase();
          
          // Direct matches
          if (extName === zenhrName || extId === zenhrId) return true;
          
          // Partial matches
          if (
            extName.includes(zenhrName) || 
            zenhrName.includes(extName) ||
            extId.includes(zenhrId) ||
            zenhrId.includes(extId)
          ) return true;
          
          // Common field patterns
          if (
            (zenhrName.includes('first') && zenhrName.includes('name') && extName.includes('first')) ||
            (zenhrName.includes('last') && zenhrName.includes('name') && extName.includes('last')) ||
            (zenhrName.includes('email') && extName.includes('email')) ||
            (zenhrName.includes('phone') && extName.includes('phone')) ||
            (zenhrName.includes('date') && extName.includes('date'))
          ) return true;
        ...
        if (potentialMatches.length > 0) {
          // Use the first match as suggestion
          suggestions[zenhrFieldId] = potentialMatches[0].id;
        }
      });
      
      setAutoMapSuggestions(suggestions);
    }
  }, [selectedFields, externalSystemFields, availableFields]);
  ...
  const getSuggestionsCount = () => {
    const existingMappings = Object.keys(mappedFields).length;
    const newSuggestions = Object.keys(autoMapSuggestions).filter(
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="alert alert-success">
          <div className="flex justify-between items-center">
            <div className="flex items-start">
              <svg className="w-5 h-5 text-green-700 mr-2 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
              <div>
                <h4 className="font-medium text-green-700">Auto-mapping Available</h4>
                <p className="text-sm text-green-600">
                  We found {getSuggestionsCount()} potential field matches based on field names.
                </p>
              </div>
            </div>
            <button
      ...
      <div className="flex items-center justify-center my-4">
        <div className="px-4 py-2 rounded-md bg-zenhr-blue-lighter text-zenhr-blue font-medium">
          {integrationDirection === 'zenhr_to_external' ? (
            <div className="flex items-center">
              ZenHR
      ...
      <div className="p-4 border border-gray-200 rounded-md">
        ...
        {Object.keys(mappedFields).length > 0 ? (
          <div className="space-y-4">
            {Object.keys(mappedFields).slice(0, 3).map(zenhrFieldId => {
              const zenhrField = zenhrFields.find(f => f.id === zenhrFieldId);
            ...
            {Object.keys(mappedFields).length > 3 && (
              <div className="text-center py-2">
                <button className="btn btn-sm btn-outline">
                  Show All Mappings ({Object.keys(mappedFields).length})
                </button>
              </div>
            )}
          </div>
